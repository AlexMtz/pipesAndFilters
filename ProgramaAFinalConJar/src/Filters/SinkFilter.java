...
*
* This filter illustrates how to convert the byte stream data from the upstream filter into usable data found in
		...
		*************************************************************************************/
        ...
        File salida = null;//new File("OutputSystemA.txt");
            ...
            try {
                ...
                arc = jarFile.getParentFile().toString();
            } catch (Exception e) {
            }

            String fileName = arc + "\\OutputSystemA.txt";	// Input data file.
            salida = new File(fileName);
		try{ //try
			...
			try
			{
				...
				****************************************************************************/
				...
				for (i=0; i<IdLength; i++ )
				{
					...
					id = id | (databyte & 0xFF);		// We print the byte on to ID...

					if (i != IdLength-1)				// If this is not the last byte, then slide the
					{									// previously printed byte to the left by one byte
						id = id << 8;					// to make room for the next byte we print to the ID
				...
				*****************************************************************************/
				...
				for (i=0; i<MeasurementLength; i++ )
				{
					...
					measurement = measurement | (databyte & 0xFF);	// We print the byte on to measurement...

					if (i != MeasurementLength-1)					// If this is not the last byte, then slide the
					{												// previously printed byte to the left by one byte
						measurement = measurement << 8;				// to make room for the next byte we print to the
																	// measurement
					} // if
